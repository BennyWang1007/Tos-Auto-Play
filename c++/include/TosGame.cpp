...
bool isEliminated(Rune *rune_list[31], int rune_indices[5][6], int x, int y) {
    int min_match = rune_list[rune_indices[y][x]]->min_match;
    if (min_match == 1)
        return true;
    int type = rune_list[rune_indices[y][x]]->type;
    if (min_match == 2) {
        if (x > 0 && type == rune_list[rune_indices[y][x-1]]->type)
            return true;
        if (x < 5 && type == rune_list[rune_indices[y][x+1]]->type)
            return true;
        if (y > 0 && type == rune_list[rune_indices[y-1][x]]->type)
            return true;
        if (y < 4 && type == rune_list[rune_indices[y+1][x]]->type)
            return true;
        return false;
    }
    if (min_match == 3) {
        if (x > 1 && type == rune_list[rune_indices[y][x-1]]->type && type == rune_list[rune_indices[y][x-2]]->type)
            return true;
        if (x < 4 && type == rune_list[rune_indices[y][x+1]]->type && type == rune_list[rune_indices[y][x+2]]->type)
            return true;
        if (y > 1 && type == rune_list[rune_indices[y-1][x]]->type && type == rune_list[rune_indices[y-2][x]]->type)
            return true;
        if (y < 3 && type == rune_list[rune_indices[y+1][x]]->type && type == rune_list[rune_indices[y+2][x]]->type)
            return true;
        if (x > 0 && x < 5 && type == rune_list[rune_indices[y][x-1]]->type && type == rune_list[rune_indices[y][x+1]]->type)
            return true;
        if (y > 0 && y < 4 && type == rune_list[rune_indices[y-1][x]]->type && type == rune_list[rune_indices[y+1][x]]->type)
            return true;
        return false;
    }
    return false;
...
// search from (x, y) to find the number of connected runes of the same type
int dfs(int x, int y, int type, int rune_indices[5][6], int to_eliminate[5][6], bool visited[5][6]) {
    // if (visited[y][x] || type != to_eliminate[y][x]) {
    //     return 0;
    // }
    visited[y][x] = true;
    to_eliminate[y][x] = 0;
    rune_indices[y][x] = 30;
    int count = 1;
    if (x > 0 && !visited[y][x-1] && to_eliminate[y][x-1] == type) {
        count += dfs(x-1, y, type, rune_indices, to_eliminate, visited);
    }
    if (x < 5 && !visited[y][x+1] && to_eliminate[y][x+1] == type) {
        count += dfs(x+1, y, type, rune_indices, to_eliminate, visited);
    }
    if (y > 0 && !visited[y-1][x] && to_eliminate[y-1][x] == type) {
        count += dfs(x, y-1, type, rune_indices, to_eliminate, visited);
    }
    if (y < 4 && !visited[y+1][x] && to_eliminate[y+1][x] == type) {
        count += dfs(x, y+1, type, rune_indices, to_eliminate, visited);
    }
    return count;
}
...
void eliminate_once_special(Rune *rune_list[31], int rune_indices[5][6], int *combo, int *total_eliminated, int *color_eliminated, int *color_combo) {
    // cout << "called eliminate_once_special\n";
    ...
    for (int y = 0; y < 5; y++) {
        for (int x = 0; x < 6; x++) {
            if (to_eliminate[y][x] != 0) {
                cur_combo++;
                target = to_eliminate[y][x];
                cur_eliminated = dfs(x, y, target, rune_indices, to_eliminate, visited);
                color_combo[target]++;
                color_eliminated[target] += cur_eliminated;
                (*total_eliminated) += cur_eliminated;
            }
...
int evaluate_score(TosGame game, int rune_indices[5][6]) {
    ...
    [[maybe_unused]] int color_eliminated[7] = {0};
    int color_combo[7] = {0};
    eliminate_once_special(game.rune_list, new_rune_indices, &combo, &total_eliminated, color_eliminated, color_combo);
    // eliminate_once(game.rune_list, new_rune_indices, &combo, &total_eliminated);
    int score = 0;
    for (int i = 1; i < 7; i++) {
